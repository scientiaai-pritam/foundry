#!/usr/bin/env node
/**
 * `foundry` CLI binary (composition root, design v1, §4 "cli").
 *
 * The kernel's `main()` builds its own context but defaults to EMPTY plugin
 * maps, so without this binary `foundry apply` / `destroy` / `migrate` find no
 ...
 *
 * Run: `foundry plan|apply|migrate|destroy` from a directory containing a
 * `foundry.config.{ts,js}`. Region resolves from AWS_REGION / AWS_DEFAULT_REGION.
 */
import { main } from "@foundry/core";
